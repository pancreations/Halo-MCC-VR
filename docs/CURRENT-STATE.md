...
- The earlier 0x2EDE38 hook alone did nothing because normal gameplay short-circuits before Halo's class-2 check. The current test build validates and removes only that short-circuit, then uses the existing class-gated predicate. It still needs headset confirmation across several weapons.
