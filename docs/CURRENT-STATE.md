...
- Resolution scale is now an isolated restart-applied test path: F1 saves resolution_scale from 0.50 to 1.00, the launcher scales Halo's 2912x2100 internal raster evenly, and the existing shader blit expands the complete eye into the unchanged full-size OpenXR projection. The 0.67 low preset still needs headset confirmation with Toolkit scaling disabled.
