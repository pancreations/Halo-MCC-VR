...
Quest 3 proved the first `feature/hud-height` control was actually curvature/depth. `hud_curvature` is now normalized: 0.00 = flat (+0.30 destination-Z delta), 1.00 = fully curved (-0.30), and 0.50 = authored. Config version 2 migrates the short-lived signed/one-tenth scale, including the old `hud_height` alias, without changing the physical curve. Each verified block retains its authored baseline so rescans cannot compound the delta. `hud_aspect` (0.50..2.00, default 1.00) trims horizontal size after automatic runtime-FOV correction. A genuinely separate `HUD height` slider writes `hud_vertical_offset` (-300..+300 virtual pixels) through a signature-located `chud_compute_anchor_basis` hook; positive raises and negative lowers the CHUD, while the separately captured authored reticle is excluded.
