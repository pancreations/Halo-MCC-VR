...
rem New ODST baseline (user decision 2026-07-22): the headset-confirmed vibration
rem build is now the frozen baseline the menu-fix test stacks onto, replacing the
rem old DFF8A406 baseline. Recovery/restore returns to this vibration DLL.
set "EXPECTED_BASELINE_DLL_SHA=B4FB36A85BFD045BEB94AD5E744AAA415DE4141C4EE31ED3D1D8C7DEE48F07EB"
...
git -C "%SRC%" branch --show-current >"%BRANCH_SNAPSHOT%" 2>nul
if errorlevel 1 (
    del /Q "%BRANCH_SNAPSHOT%" >nul 2>&1
    echo *** FAILED: the current branch could not be read.
    goto :abort_no_install
)
set /p "TEST_BRANCH="<"%BRANCH_SNAPSHOT%"
del /Q "%BRANCH_SNAPSHOT%" >nul 2>&1
...
git -C "%SRC%" rev-parse HEAD >"%HEAD_SNAPSHOT%" 2>nul
if errorlevel 1 (
    del /Q "%HEAD_SNAPSHOT%" >nul 2>&1
    echo *** FAILED: the source commit identity could not be read.
    goto :abort_no_install
)
set /p "TEST_COMMIT="<"%HEAD_SNAPSHOT%"
del /Q "%HEAD_SNAPSHOT%" >nul 2>&1
...
if errorlevel 1 goto :abort_backup_creation
...
if errorlevel 1 goto :abort_private_staging
...
if errorlevel 1 goto :recover_install_failure
...
if errorlevel 1 goto :restore_not_proven
if exist "%STATE_RESTORING%" goto :restore_not_proven
if not exist "%BACKUP_DIR%\RESTORED.txt" goto :restore_not_proven
echo.
...
set /p "CURRENT_BRANCH="<"%BRANCH_SNAPSHOT%"
del /Q "%BRANCH_SNAPSHOT%" >nul 2>&1
if not defined CURRENT_BRANCH (
    echo *** FAILED: the current branch identity is empty.
    exit /b 1
)
if /I not "%CURRENT_BRANCH%"=="feature/odst-bringup" (
...
if errorlevel 1 (
    del /Q "%HEAD_SNAPSHOT%" >nul 2>&1
    echo *** FAILED: HEAD identity can no longer be read.
    exit /b 1
)
set /p "CURRENT_HEAD="<"%HEAD_SNAPSHOT%"
del /Q "%HEAD_SNAPSHOT%" >nul 2>&1
if not defined CURRENT_HEAD (
    echo *** FAILED: the current HEAD identity is empty.
    exit /b 1
)
if /I not "%CURRENT_HEAD%"=="%TEST_COMMIT%" (
...
call :require_game_closed
if errorlevel 1 goto :rollback_not_proven
call :prepare_restore_state
if errorlevel 1 goto :rollback_not_proven
copy /Y "%BASELINE_DLL%" "%DLL_DST%" >nul
if errorlevel 1 goto :rollback_not_proven
...
if errorlevel 1 goto :rollback_not_proven
if exist "%STATE_RESTORING%" goto :rollback_not_proven
if not exist "%BACKUP_DIR%\ROLLED_BACK.txt" goto :rollback_not_proven
