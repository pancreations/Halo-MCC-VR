"""Apply only the proven Halo 4 theatre-camera delta to exact Stage 3BW."""
...
# Stage 3BW ends at 0x2FACA9. Stage 3BX used this aligned tail without
# corrupting the earlier reticle-probe string terminator at 0x2FA36C.
PAYLOAD_RVA = 0x002FACB0
PAYLOAD_LIMIT = 0x002FAD50
...
def build_fixed_payload():
    e = Encoder()
    e.emit("48 83 EC 28 48 89 4C 24 20")
    ...
    e.emit("0F 2E C0")
    e.rel8("7B", "finite")
    e.emit("B8 00 00 80 3F 66 0F 6E C0")
    e.rel8("EB", "scale")
    e.label("finite")
    e.emit("0F 57 C9 F3 0F 5F C1")
    ...
    return e.finish(), {"s3cc_get_render_snapshot": PAYLOAD_RVA}
...
def main():
    ...
    if digest != EXPECTED_INPUT_SHA256:
        raise SystemExit("wrong Stage 3BW input DLL: " + digest)
    pe = parse_pe(blob)
    ...
    thunk = symbols["s3cc_get_render_snapshot"]
