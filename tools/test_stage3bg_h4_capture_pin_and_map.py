...
OUT = (ROOT / "built" / "Stage3BG-HaloMCCVR.dll").read_bytes()
EXPECTED = "1ff5f99724f793cb3a51cc2551d017c72f2fabae40cc0c5dfacf63e56b233355"

PAYLOAD_RVA = 0x2FB810
PAYLOAD_END = 0x2FB810 + 1517
GATE_RVA = 0x2FB850
TAP_RVA = 0x2FB9ED
DUMP_RVA = 0x2FBA69
...
assert rip_target(DUMP_RVA + 0x0C, 7, 3) == 0x2AE440, \
    "dump reads g_authoredReticleTexture"
assert rip_target(0x2FBA85, 7, 3) == 0x2AE298, "dump reads g_context"
assert read(OUT, out_s, 0x2FBAA2, 6) == bytes.fromhex("488b01ff5050"), \
    "GetDesc via texture vtable +0x50"
assert read(OUT, out_s, 0x2FBAA8, 8) == \
    bytes.fromhex("c744245c03000000"), "desc.Usage = STAGING"
assert read(OUT, out_s, 0x2FBAB8, 8) == \
...
assert rip_target(0x2FBAC8, 7, 3) == 0x2AE290, "dump reads g_device"
assert read(OUT, out_s, 0x2FBAF1, 3) == bytes.fromhex("ff5028"), \
    "CreateTexture2D via device vtable +0x28"
assert read(OUT, out_s, 0x2FBB18, 6) == bytes.fromhex("ff9078010000"), \
    "CopyResource via context vtable +0x178"
assert read(OUT, out_s, 0x2FBB43, 3) == bytes.fromhex("ff5070"), \
    "Map via context vtable +0x70"
assert read(OUT, out_s, 0x2FBC1D, 3) == bytes.fromhex("ff5078"), \
    "Unmap via context vtable +0x78"
assert read(OUT, out_s, 0x2FBBEB, 4) == bytes.fromhex("4183fb20"), \
    "column loop bound 32"
assert read(OUT, out_s, 0x2FBC08, 3) == bytes.fromhex("83ff20"), \
    "row loop bound 32"

# 5. Stage 3BD identity intact underneath.
...
print("PASS Stage3BG pin + telemetry + one-shot capture map; all three "
