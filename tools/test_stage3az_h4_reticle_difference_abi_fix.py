...
OUT = ROOT / "built" / "Stage3AZ-HaloMCCVR.dll"
EXPECTED_BASE = "0d32751585670c28bb7b98110a35b04817ec4f683fcc3ab3301c0941a4613053"
EXPECTED_OUT = "76727356f3ce4b053b09eccde6783a82ea5249d7ebfb9df9bc949b8822affb10"
...
payload = read(0x2FB000, 2072)
assert hashlib.sha256(payload).hexdigest() == \
    "a4c2f2ee62cc237b0887fd260b89001caf13ba10a617f11d3c25583a25016316"
shader_offset = payload.index(b"DXBC")
assert len(payload) - shader_offset == 1000

# The crashing AY wrapper nested a compiled call without a Windows x64 call
# frame. AZ tail-transfers both capture-begin branches and brackets SafeWrite
# with aligned 0x20-byte shadow space. It also leaves nonvolatile RBX intact.
assert read(0x2FB008, 5)[0] == 0xE9
assert read(0x2FB017, 5)[0] == 0xE9
finish = read(0x2FB02D, 0x68)
assert bytes.fromhex("48 83 ec 28 e8") in finish
assert bytes.fromhex("48 83 c4 28") in finish
select = read(0x2FB12C, 0x1AF)
assert bytes.fromhex("48 89 5c 24 28") in select
assert bytes.fromhex("48 8b 54 24 28") in select
assert bytes.fromhex("48 8b 1d") not in select
...
print("PASS Stage3AZ H4 reticle differential ABI fix + ODST optional-scan isolation")
