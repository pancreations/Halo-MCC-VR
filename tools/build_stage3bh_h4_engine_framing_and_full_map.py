...
# The engine's own measured CUI framing (S3BG telemetry, 2026-08-28 18:44):
# 4 x the live CUI half-extents 1033.578 / 336.549, which Begin's (512-W)/2
# centring turns into exactly tl=(-1811.16, -417.10) wh=4134.31x1346.20.
CONST_RVA = 0x002FB800
NEW_VIEWPORT = struct.pack("<ffff", 4134.312, 1346.196, 0.0, 1.0)
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: build_stage3bh_h4_capture_pin_and_map.py "
    ...
    if any(blob[o:rva_off(pe, PAYLOAD_LIMIT - 1) + 1]):
        raise SystemExit("Stage3BH payload region is not free")

    code, symbols = build_payload(
        Path(__file__).parent / "stage3bh_h4_engine_framing_and_full_map.S",
        PAYLOAD_RVA,
        defs=dict(
            LOG_RVA=LOG_RVA,
        ...
        want_symbols=("stage3bh_data", "s3bh_draw_gate", "s3bh_upload_tap"))
    if PAYLOAD_RVA + len(code) > PAYLOAD_LIMIT:
        raise SystemExit(f"payload too large: {len(code)} bytes")
    gate_rva = symbols["s3bh_draw_gate"]
    tap_rva = symbols["s3bh_upload_tap"]
    ...
    # Adopt the engine's own measured CUI framing.  The 18:44 telemetry read
    # tl=(-1811,-417) wh=4134x1346 on every captured draw the engine itself
    # framed; that rect is centred exactly on the 512x512 texture and is
    # exactly Begin's own (512-W)/2 formula fed 4x the live CUI half-extents.
    # Writing those numbers into the private constant makes Begin, the
    # existing reroute re-assert and the draw-time gate all agree with the
    # engine, and frames +/-128 authored units square (reticle = 28.9%,
    # matching Halo 3 / ODST occupancy) instead of +/-258 x +/-84.
    const_off = rva_off(pe, CONST_RVA)
    blob[const_off:const_off + 16] = NEW_VIEWPORT
    w, h, mind, maxd = struct.unpack("<ffff", NEW_VIEWPORT)
    print(f"capture framing constant 0x{CONST_RVA:X}: "
          f"{{2048,2048}} -> {{{w},{h}}}  "
          f"(TopLeft {(512 - w) * 0.5:.1f},{(512 - h) * 0.5:.1f})")
    if abs((512 - w) * 0.5 + 1811.156) > 0.01 or \
            abs((512 - h) * 0.5 + 417.098) > 0.01:
        raise SystemExit("constant does not reproduce the measured viewport")
